...
bool ArduinoSerial::connectToPort(const QString& portName, int baudRate, QString* errorOut)
{
    ...
    if (!m_port.open(QIODevice::ReadWrite)) {
        ...
        // Ne pas émettre errorOccurred si errorOut est fourni (pour éviter les doublons)
        if (!errorOut) {
            emit errorOccurred(msg);
        }
        return false;
    ...
    // Many Arduino boards reset when the serial port opens; we keep this class
    // generic and do not block here.
...
bool ArduinoSerial::writeLine(const QString& line, QString* errorOut)
{
    ...
    // Ne PAS attendre waitForBytesWritten - ça cause des problèmes avec Arduino !
    return true;
#else
    Q_UNUSED(line);
    const QString msg = QStringLiteral("Qt SerialPort indisponible (module non installe).");
