...
void MainWindow::setupArduinoIntegration()
{
    ...
    if (auto* sb = statusBar()) {
        // Ne pas ajouter le label Arduino à la barre de statut
    ...
    // --- NOUVELLE PARTIE : Connexion automatique et cacher les UI ---
    // Cacher les éléments UI que vous ne voulez plus voir
    if (m_arduinoStatusLabel) m_arduinoStatusLabel->hide();
    if (m_arduinoPortCombo) m_arduinoPortCombo->hide();
    if (m_arduinoConnectButton) m_arduinoConnectButton->hide();
    if (m_arduinoTxEdit) m_arduinoTxEdit->hide();
    if (m_arduinoSendButton) m_arduinoSendButton->hide();
    ...
    QTimer::singleShot(500, this, [this]() {
        ...
        QString portName;
        if (m_arduinoPortCombo && m_arduinoPortCombo->count() > 0) {
            // Essayer d'abord le dernier port utilisé
            if (m_arduinoPortCombo->currentText() != "(no ports)") {
                portName = m_arduinoPortCombo->currentText().trimmed();
            }
            // Si pas, prendre le premier port disponible
            if (portName.isEmpty() || portName == "(no ports)") {
                for (int i = 0; i < m_arduinoPortCombo->count(); ++i) {
                    const QString p = m_arduinoPortCombo->itemText(i).trimmed();
                    if (p != "(no ports)") {
                        portName = p;
                        m_arduinoPortCombo->setCurrentIndex(i);
                        break;
                    }
                }
            }
        ...
        if (portName.isEmpty() || portName == "(no ports)") {
            if (m_portConsole) {
                m_portConsole->appendPlainText("[INIT] ❌ Aucun port série trouvé pour la connexion automatique");
            }
            return;
        }

        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[INIT] Tentative de connexion automatique à %1...").arg(portName));
        }

        QString error;
        if (!m_arduino->connectToPort(portName, 9600, &error)) {
            if (m_portConsole) {
                m_portConsole->appendPlainText(QStringLiteral("[INIT] ❌ Échec de la connexion automatique: %1").arg(error));
            }
        }
    });
    ...
    connect(m_arduino, &ArduinoSerial::connected, this, [this](const QString& portName) {
        ...
        // Reset baseline calibration/counters
        ...
        if (m_portConsole) {
            m_portConsole->appendPlainText("[STATE] ✓ Variables réinitialisées: bateau_present=FALSE, docked=FALSE");
            m_portConsole->appendPlainText("[STATE] ✓ Baseline reset (waiting samples)");
        }
        ...
        if (m_portConsole) {
            ...
            m_portConsole->appendPlainText("[CONFIG] Seuils détection: DETECT=210cm | DOCK=100cm | DEPART=220cm");
            m_portConsole->appendPlainText("[INFO] Pour modifier les seuils, édite mainwindow.cpp ligne ~5862");
        }

        // === INITIALIZE DISPLAY TO "Port en attente" ===
        QTimer::singleShot(500, this, [this]() {
            if (m_arduino) {
                m_arduino->writeLine("DISPLAY:Port en attente");
                if (m_portConsole) m_portConsole->appendPlainText("[QT] Command: DISPLAY:Port en attente → Afficheur initialized");
            }
        });
    });
    ...
    connect(m_arduino, &ArduinoSerial::disconnected, this, [this]() {
        ...
        updateArduinoUiState();
        updateArduinoTemperatureDialogConnectionState();
        // Ensure Arduino sensor stopped on disconnect
        if (m_arduino) {
            m_arduino->writeLine("STOP");
            if (m_portConsole) m_portConsole->appendPlainText("[QT] Arduino disconnected: Command STOP sent → sensor INACTIVE");
        } else {
...
void MainWindow::handleArduinoPortLine(const QString& line)
{
    ...
    if (msgOrig.startsWith("PULSE:")) {
        ...
        const int DETECT = 210;   // Boat detected (fallback)
        const int DOCK = 100;     // Boat at dock
        const int DEPART = 220;   // Boat left

        // Baseline calibration + consecutive-reads filter to avoid false positives
        const int BASELINE_SAMPLES_REQUIRED = 8;
        const int BASELINE_DELTA_CM = 40;     // must be this much closer than baseline
        const int REQUIRED_CONSECUTIVE_BELOW = 4;

        if (m_baselineDistanceCm <= 0) {
            // Accumulate initial baseline samples (only when sensor reports valid readings)
            m_baselineSum += distCm;
            m_baselineSamples++;
            if (m_baselineSamples >= BASELINE_SAMPLES_REQUIRED) {
                m_baselineDistanceCm = int(double(m_baselineSum) / double(m_baselineSamples));
                if (m_portConsole) m_portConsole->appendPlainText(QStringLiteral("[STATE] Baseline ready: %1cm (from %2 samples)")
                    .arg(m_baselineDistanceCm).arg(m_baselineSamples));
            } else {
                if (m_portConsole) m_portConsole->appendPlainText(QStringLiteral("[STATE] Baseline sampling: %1/%2 (current avg=%3cm)")
                    .arg(m_baselineSamples).arg(BASELINE_SAMPLES_REQUIRED).arg(int(double(m_baselineSum)/double(m_baselineSamples))));
            }
        }

        // Determine if this reading is considered 'below' threshold (closer)
        bool isBelow = false;
        if (m_baselineDistanceCm > 0) {
            isBelow = (distCm < (m_baselineDistanceCm - BASELINE_DELTA_CM));
        } else {
            // Baseline not ready: do not declare arrival yet to avoid false positives
            isBelow = false;
        }

        // Track consecutive below readings
        if (isBelow) m_consecutiveBelow++; else m_consecutiveBelow = 0;

        // Track readings that are back near the baseline, which means the boat moved away
        const int DEPART_MARGIN_CM = 20;
        bool isBackToIdle = false;
        if (m_baselineDistanceCm > 0) {
            isBackToIdle = (distCm >= (m_baselineDistanceCm - DEPART_MARGIN_CM));
        }
        if (isBackToIdle) m_consecutiveAbove++; else m_consecutiveAbove = 0;

        // ARRIVAL DETECTION (require several consecutive below readings)
        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[STATE] Vérif ARRIVAL: distance=%1cm | baseline=%2cm | below=%3 | consecutive=%4 | bateau_present=%5")
                .arg(distCm).arg(m_baselineDistanceCm).arg(isBelow ? "OUI" : "NON").arg(m_consecutiveBelow).arg(m_boatPresent ? "OUI" : "NON"));
        }

        if (m_consecutiveBelow >= REQUIRED_CONSECUTIVE_BELOW && !m_boatPresent) {
            m_boatPresent = true;
            m_boatDocked = false;
            m_currentAssignedQuaiId = -1;

            // Record arrival timestamp to avoid immediate docking noise
            m_arrivalDetectedAt = QDateTime::currentMSecsSinceEpoch();

            if (m_portConsole) m_portConsole->appendPlainText("[STATE] ✓ ARRIVAL DETECTÉE -> Appel findAndReserveFreeQuai()");
            if (m_arduino) m_arduino->writeLine("BATEAU_DETECTE");

            QTimer::singleShot(700, this, [this]() {
                if (!m_boatPresent) return;
                const int quaiId = findAndReserveFreeQuai();

                if (m_portConsole) {
                    m_portConsole->appendPlainText(
                        QStringLiteral("[DECISION] findAndReserveFreeQuai() retourné: %1").arg(quaiId));
                }

                if (quaiId > 0) {
                    m_currentAssignedQuaiId = quaiId;
                    if (m_portConsole) {
                        m_portConsole->appendPlainText(QStringLiteral("[COMMAND] ✓ QUAI:%1 → Afficheur").arg(quaiId));
                    }
                    if (m_arduino) m_arduino->writeLine(QStringLiteral("QUAI:%1").arg(quaiId));
                } else {
                    if (m_portConsole) {
                        m_portConsole->appendPlainText("[COMMAND] ❌ COMPLET → Afficheur (retour findAndReserveFreeQuai = -1)");
                    }
        ...
        if (distCm <= DOCK && m_boatPresent && !m_boatDocked) {
            // Require a short delay after arrival to confirm docking (filter noise)
            const qint64 nowMs = QDateTime::currentMSecsSinceEpoch();
            const qint64 sinceArrival = (m_arrivalDetectedAt > 0) ? (nowMs - (qint64)m_arrivalDetectedAt) : INT64_MAX;
            const qint64 DOCK_CONFIRM_MS = 800; // wait 800ms after arrival before declaring docked
            if (sinceArrival < DOCK_CONFIRM_MS) {
                if (m_portConsole) m_portConsole->appendPlainText(QStringLiteral("[STATE] Dock check skipped (sinceArrival=%1ms < %2ms)").arg(sinceArrival).arg(DOCK_CONFIRM_MS));
                return;
            }

            m_boatDocked = true;
            if (m_portConsole) m_portConsole->appendPlainText("[STATE] DOCKED");
            // Keep the assigned quai display visible; do not overwrite it with a docked message.
            if (m_portConsole) m_portConsole->appendPlainText("[STATE] Docked state reached: display quai ID kept intact");
            return;
        }

        // DEPARTURE DETECTION
        const int REQUIRED_CONSECUTIVE_ABOVE = 3;
        if (m_boatPresent && m_consecutiveAbove >= REQUIRED_CONSECUTIVE_ABOVE) {
            m_boatPresent = false;
            m_boatDocked = false;
            if (m_portConsole) m_portConsole->appendPlainText("[STATE] DEPARTED");
            // Inform Arduino and display immediately that the boat has left
            if (m_arduino) {
                m_arduino->writeLine("BATEAU_PARTI");
                m_arduino->writeLine("DISPLAY:Bateau parti");
            }

            // Free the assigned quai in DB and refresh UI
            if (m_currentAssignedQuaiId > 0) {
                if (m_portConsole) {
                    m_portConsole->appendPlainText(QStringLiteral("[STATE] Libération quai assigné: %1").arg(m_currentAssignedQuaiId));
                }
                markQuaiFree(m_currentAssignedQuaiId);
                m_currentAssignedQuaiId = -1;
            } else if (m_portConsole) {
                m_portConsole->appendPlainText("[STATE] ⚠ Aucun quai assigné à libérer au départ");
            }

            refreshQuaiTable();

            // After a short pause, restore idle message
            QTimer::singleShot(2000, this, [this]() {
                if (m_arduino) m_arduino->writeLine("DISPLAY:Port en attente");
            });
            return;
        }
    }
...
void MainWindow::debugAfficherTousLesQuais()
{
    Connection* conn = Connection::getInstance();
    if (!conn->ensureOpen()) {
        if (m_portConsole) m_portConsole->appendPlainText("[DEBUG] ❌ Impossible ouvrir BD");
        return;
    }
    ...
    // --- CLEANUP: corriger les lignes incohérentes où STATUT='LIBRE' mais ID_BATEAU non NULL ---
    ...
    if (cleanup.exec(cleanupSql)) {
        long cleaned = cleanup.numRowsAffected();
        if (m_portConsole) m_portConsole->appendPlainText(QStringLiteral("[DB] Cleanup: ID_BATEAU remis à NULL pour %1 lignes inconsistantes (STATUT='LIBRE')").arg(cleaned));
    } else {
        if (m_portConsole) m_portConsole->appendPlainText(QStringLiteral("[DB] Cleanup échoué: %1").arg(cleanup.lastError().text()));
    }

    if (m_portConsole) {
        m_portConsole->appendPlainText("\n[DEBUG] ========== ÉTAT TABLE QUAIS ==========");
    }

    if (!q.exec("SELECT ID_QUAI, STATUT, ID_BATEAU, NOM_QUAI FROM QUAIS ORDER BY ID_QUAI")) {
        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[DEBUG] ❌ Erreur SELECT: %1").arg(q.lastError().text()));
        }
        return;
    }
    ...
    while (q.next()) {
        ...
        QString statut = q.value(1).toString();
        int idBateau = q.value(2).toInt();
        QString nom = q.value(3).toString();

        QString bateau = (idBateau > 0) ? QString::number(idBateau) : "NULL";
        
        // Compte les statuts
        QString statutUpper = statut.trimmed().toUpper();
        if (statutUpper == "OCCUPE" || statutUpper == "OCCUPEE") occupeCount++;
        else if (statutUpper == "LIBRE") libreCount++;
        else if (statut.trimmed().isEmpty()) nullCount++;

        if (m_portConsole) {
            m_portConsole->appendPlainText(
                QStringLiteral("[QUAI] #%1: STATUT='%2' (upper='%3') BATEAU=%4 NOM='%5'")
                .arg(id).arg(statut).arg(statutUpper).arg(bateau).arg(nom));
        }
    }

    if (rowCount == 0) {
        if (m_portConsole) {
            m_portConsole->appendPlainText("[DEBUG] ⚠️  TABLE VIDE - AUCUN QUAI!");
        }
    } else {
        if (m_portConsole) {
            m_portConsole->appendPlainText(
                QStringLiteral("[DEBUG] RÉSUMÉ: %1 quais | OCCUPE=%2 | LIBRE=%3 | NULL=%4")
                .arg(rowCount).arg(occupeCount).arg(libreCount).arg(nullCount));
        }
    }

    // Test la requête elle-même
    if (m_portConsole) {
        m_portConsole->appendPlainText("[DEBUG] Test WHERE TRIM(UPPER(STATUT)) = 'LIBRE'...");
    }

    QSqlQuery testQ(db);
    testQ.prepare("SELECT ID_QUAI, STATUT FROM QUAIS WHERE NVL(ID_BATEAU,0)=0 AND TRIM(UPPER(STATUT)) = 'LIBRE' AND ROWNUM <= 1");
    if (testQ.exec() && testQ.next()) {
        int testId = testQ.value(0).toInt();
        QString testStatut = testQ.value(1).toString();
        if (m_portConsole) {
            m_portConsole->appendPlainText(
                QStringLiteral("[DEBUG] ✓ Requête trouvé: ID=%1 STATUT='%2'").arg(testId).arg(testStatut));
        }
    } else {
        if (m_portConsole) {
            m_portConsole->appendPlainText("[DEBUG] ❌ Requête retourné ZÉRO lignes pour STATUT='LIBRE'!");
        }
    }

    if (m_portConsole) {
        m_portConsole->appendPlainText("[DEBUG] =====================================\n");
    }
}
...
{
    if (m_portConsole) {
        m_portConsole->appendPlainText("\n[=== FINDANDRESERVEQUAI() DEBUT ===]");
    }

    // Affiche l'état complet de la table QUAIS avant de chercher
    debugAfficherTousLesQuais();

    Connection* conn = Connection::getInstance();
    if (!conn->ensureOpen()) {
        if (m_portConsole) m_portConsole->appendPlainText("[DB] ❌ Erreur: BD non ouverte");
        return -1;
    }
    ...
    // STRATEGIE: Chercher uniquement les quais réellement libres
    // Convention de l'app: Libre / Occupe / Ferme
    ...
    if (m_portConsole) {
        m_portConsole->appendPlainText(QStringLiteral("[DB] Exécution requête: %1").arg(sql));
    }

    // Count matching rows for debug
    QSqlQuery countQ(db);
    ...
    if (countQ.exec(countSql) && countQ.next()) {
        int matchCount = countQ.value(0).toInt();
        if (m_portConsole) m_portConsole->appendPlainText(QStringLiteral("[DB] Matching free quais count: %1").arg(matchCount));
    } else {
        if (m_portConsole) m_portConsole->appendPlainText(QStringLiteral("[DB] Count query failed: %1").arg(countQ.lastError().text()));
    }

    if (!q.exec(sql)) {
        qDebug() << "Erreur requete free quai:" << q.lastError().text();
        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[DB] ❌ ERREUR SQL: %1").arg(q.lastError().text()));
        }
        if (m_portConsole) m_portConsole->appendPlainText("[=== FINDANDRESERVEQUAI() RETURN -1 (erreur SQL) ===]\n");
        return -1;
    }

    if (!q.next()) {
        if (m_portConsole) {
            m_portConsole->appendPlainText("[DB] ❌ AUCUN QUAI NE CORRESPOND À LA CONDITION: STATUT != 'OCCUPE'");
            m_portConsole->appendPlainText("[=== FINDANDRESERVEQUAI() RETURN -1 (aucun quai libre) ===]\n");
        }
        return -1;
    }
    ...
    if (m_portConsole) {
        m_portConsole->appendPlainText(QStringLiteral("[DB] ✓ QUAI LIBRE TROUVÉ: ID=%1").arg(id));
    }

    // Reserve it: set statut to 'Occupe' (exact value from check constraint)
    QSqlQuery up(db);
    up.prepare("UPDATE QUAIS SET STATUT = ? WHERE ID_QUAI = ?");
    up.addBindValue(QStringLiteral("Occupe"));
    up.addBindValue(id);
    
    if (m_portConsole) {
        m_portConsole->appendPlainText(QStringLiteral("[DB] Mise à jour: QUAI %1 → SET STATUT='Occupe' (ID_BATEAU inchangé)").arg(id));
    }

    if (!up.exec()) {
        qDebug() << "Erreur reservation quai:" << up.lastError().text();
        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[DB] ❌ ERREUR UPDATE: %1").arg(up.lastError().text()));
            m_portConsole->appendPlainText("[=== FINDANDRESERVEQUAI() RETURN -1 (erreur update) ===]\n");
        }
        return -1;
    }
    if (m_portConsole) {
        m_portConsole->appendPlainText(QStringLiteral("[STATE] ✓ QUAI %1 RÉSERVÉ ET MARQUÉ Occupe").arg(id));
        m_portConsole->appendPlainText(QStringLiteral("[=== FINDANDRESERVEQUAI() RETURN %1 (succès) ===]\n").arg(id));
    }

    // Update UI table to reflect the new statut
    refreshQuaiTable();
    return id;
}
...
void MainWindow::markQuaiFree(int idQuai)
{
    ...
    if (m_portConsole) {
        m_portConsole->appendPlainText(QStringLiteral("[DB] Libération quai %1...").arg(idQuai));
    }

    QSqlQuery q(db);
    q.prepare("UPDATE QUAIS SET STATUT = ?, ID_BATEAU = NULL WHERE ID_QUAI = ?");
    q.addBindValue(QStringLiteral("Libre"));
    q.addBindValue(idQuai);
    if (!q.exec()) {
        qDebug() << "Erreur liberation quai:" << q.lastError().text();
        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[DB] Erreur liberation quai %1: %2").arg(idQuai).arg(q.lastError().text()));
        }
    } else {
        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[DB] UPDATE LIBERATION rows=%1").arg(q.numRowsAffected()));
        }

        QSqlQuery verify(db);
        verify.prepare("SELECT STATUT, ID_BATEAU FROM QUAIS WHERE ID_QUAI = ?");
        verify.addBindValue(idQuai);
        if (verify.exec() && verify.next()) {
            const QString statut = verify.value(0).toString();
            const QString idBateau = verify.value(1).isNull() ? QStringLiteral("NULL") : verify.value(1).toString();
            if (m_portConsole) {
                m_portConsole->appendPlainText(QStringLiteral("[DB] Vérif quai %1 => STATUT='%2' ID_BATEAU=%3")
                                               .arg(idQuai).arg(statut).arg(idBateau));
            }
        } else if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[DB] Vérif quai %1 impossible: %2").arg(idQuai).arg(verify.lastError().text()));
        }

        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[STATE] DEPARTED => Quai %1 LIBERE (LIBRE)").arg(idQuai));
        }
        // Refresh UI to show quai liberated
        refreshQuaiTable();
    }
}
...
void MainWindow::on_pushButton_11_clicked()
{
    ...
    QString statutDb;
    if (statutUi.startsWith(QStringLiteral("Occu"), Qt::CaseInsensitive)) {
        statutDb = QStringLiteral("Occupe");
    } else if (statutUi.startsWith(QStringLiteral("Lib"), Qt::CaseInsensitive)) {
        statutDb = QStringLiteral("Libre");
    } else if (statutUi.startsWith(QStringLiteral("Main"), Qt::CaseInsensitive) || statutUi.startsWith(QStringLiteral("Ferm"), Qt::CaseInsensitive)) {
        statutDb = QStringLiteral("Maintenance");
    } else {
        statutDb = statutUi;
    }
    ...
    if (statutDb.startsWith(QStringLiteral("OCCU"), Qt::CaseInsensitive) && idBateau <= 0) {
        QMessageBox::warning(this,
    ...
    if (m_quaisFermeMeteo && savedQuaiId > 0) {
        if (statutDb.startsWith(QStringLiteral("OCCU"), Qt::CaseInsensitive)) {
...
void MainWindow::lockQuaisForWeather()
{
    ...
    if (selectQuery.exec(QStringLiteral("SELECT ID_QUAI, STATUT, ID_BATEAU FROM QUAIS"))) {
        while (selectQuery.next()) {
            ...
            if (statut.compare(QStringLiteral("Occupe"), Qt::CaseInsensitive) == 0
                || statut.compare(QStringLiteral("Occupee"), Qt::CaseInsensitive) == 0) {
    ...
    updateQuery.prepare(QStringLiteral(
        ...
        "WHERE UPPER(STATUT) IN ('OCCUPE', 'OCCUPEE')"));
    updateQuery.exec();
}
...
void MainWindow::unlockQuaisForWeather()
{
    ...
    fallbackQuery.exec(QStringLiteral(
        ...
        "WHERE UPPER(STATUT) = 'MAINTENANCE'"));
}
