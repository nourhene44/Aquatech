...
void MainWindow::setupArduinoIntegration()
{
    ...
    if (!m_portConsole) {
        ...
        if (auto* sb = statusBar()) {
            ...
            m_portConsole->hide(); // Hide the log console
        }
    }

    m_portConsole->appendPlainText(QStringLiteral("[INIT] Console logs Arduino/Database"));
    ...
    connect(m_arduino, &ArduinoSerial::connected, this, [this](const QString& portName) {
        ...
        if (m_portConsole) {
            ...
            m_portConsole->appendPlainText("[WAIT] Attente 2s pour reset Arduino...");
        }

        // Attendre 2 secondes pour que l'Arduino termine son reset
        QTimer::singleShot(2000, this, [this]() {
            if (!m_arduino) return;

            // === SEND START COMMAND TO ACTIVATE SENSOR ===
            m_arduino->writeLine("START");
            if (m_portConsole) {
    ...
    connect(m_arduino, &ArduinoSerial::errorOccurred, this, [this](const QString& message) {
        // Ne pas afficher l'erreur "délai de sémaphore" du tout (c'est normal avec Arduino)
        if (!message.contains("sémaphore") && !message.contains("semaphore")) {
            if (statusBar()) statusBar()->showMessage(QStringLiteral("Arduino error: %1").arg(message), 8000);
            if (m_portConsole) {
                m_portConsole->appendPlainText(QStringLiteral("[ERR] Arduino: %1").arg(message));
            }
        }
        updateArduinoUiState();
        updateArduinoTemperatureDialogConnectionState();
    ...
    // --- CONNEXION AUTOMATIQUE ET CACHER LES UI ---
    // Cacher les éléments UI Arduino
    ...
    // Timer pour les retentatives de connexion
    m_arduinoReconnectTimer = new QTimer(this);
    m_arduinoReconnectTimer->setSingleShot(true);
...
void MainWindow::attemptArduinoConnection()
{
    ...
    // FORCER LA CONNEXION SUR COM6 !
    QString portName = "COM6";
    if (m_portConsole) {
        m_portConsole->appendPlainText(QStringLiteral("[INIT] FORÇAGE CONNEXION SUR %1 (essai %2/10)...").arg(portName).arg(m_arduinoReconnectAttempts + 1));
    }

    QString error;
    if (!m_arduino->connectToPort(portName, 9600, &error)) {
        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[INIT] ❌ Échec sur %1: %2").arg(portName).arg(error));
            m_portConsole->appendPlainText(QStringLiteral("[INIT] → VÉRIFIEZ : 1) Arduino IDE FERMÉ  2) Arduino branché sur %1").arg(portName));
        }
        // Réessayer dans 2 secondes
        if (m_arduinoReconnectAttempts < 10) {
            m_arduinoReconnectAttempts++;
            m_arduinoReconnectTimer->start(2000);
        }
    } else {
        m_arduinoReconnectAttempts = 0;
        if (m_portConsole) {
            m_portConsole->appendPlainText(QStringLiteral("[INIT] ✓ CONNEXION RÉUSSIE SUR %1 !").arg(portName));
        }
    }
}
...
void MainWindow::handleArduinoPortLine(const QString& line)
{
    ...
    // Parse either PULSE: (raw microseconds, Qt calculates distance) OR DISTANCE: (Arduino calculates distance)
    ...
    } else if (msgOrig.startsWith("DISTANCE:")) {
        bool ok = false;
        distCm = msgOrig.mid(9).toInt(&ok);
        if (!ok) return;
    ...
    const int BASELINE_SAMPLES_REQUIRED = 5;
    const int BASELINE_DELTA_CM = 20;
    const int REQUIRED_CONSECUTIVE_BELOW = 3;
    ...
    if (distCm <= DOCK && m_boatPresent && !m_boatDocked) {
        ...
        const qint64 DOCK_CONFIRM_MS = 800;
