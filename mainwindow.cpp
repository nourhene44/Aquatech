...
void MainWindow::refreshTopClientsStats()
{
    ...
    // On filtre par profil 'Fidele' (ou équivalent) et on trie par date d'inscription
    ...
    QSqlQuery query(db);
    if (!query.exec(sql)) {
        qWarning() << "refreshTopClientsStats query failed:" << query.lastError().text() << "SQL:" << sql;
        return;
    }
    ...
    while (query.next() && row < 5) {
        ...
        for (int col = 0; col < 5; ++col) {
            QString val;
            if (col == 4) {
                // Pour la colonne "Date Inscription", on affiche l'ancienneté (Date) ou un label
                val = query.value(col).toDate().toString(QStringLiteral("dd/MM/yyyy"));
                if (val.isEmpty()) val = QStringLiteral("Fidèle");
            } else {
...
void MainWindow::setupArduinoIntegration()
{
    ...
    QSettings settings(QStringLiteral("AquaTech"), QStringLiteral("AquaTech"));
    settings.setValue(QStringLiteral("arduino/rfidPortName"), QStringLiteral("COM4"));
    settings.setValue(QStringLiteral("arduino/boatPortName"), QStringLiteral("COM6"));
    settings.setValue(QStringLiteral("arduino/tempPortName"), QStringLiteral("COM7"));
    settings.sync();
    const QString savedPort = QStringLiteral("COM4");
    ...
    QTimer::singleShot(350, this, [this]() {
        ...
        // Ports are available - stop the retry timer
        if (m_arduinoPortDetectionTimer && m_arduinoPortDetectionTimer->isActive()) {
            m_arduinoPortDetectionTimer->stop();
        }
        ...
        if (portName.isEmpty()) {
            if (statusBar()) {
                statusBar()->showMessage(QStringLiteral("Arduino: Could not select a port from available ports."), 6000);
            }
            updateArduinoUiState();
            return;
        }
...
{
    if (statusBar()) {
        statusBar()->showMessage(QStringLiteral("Telegram Remise cliqué (non implémenté)"), 3000);
    }
}
...
{
    if (statusBar()) {
        statusBar()->showMessage(QStringLiteral("Ajouter ligne cliqué (non implémenté)"), 3000);
    }
}
...
{
    if (statusBar()) {
        statusBar()->showMessage(QStringLiteral("Supprimer ligne cliqué (non implémenté)"), 3000);
    }
}
...
{
    if (statusBar()) {
        statusBar()->showMessage(QStringLiteral("Bouton capture cliqué (non implémenté)"), 3000);
    }
}
...
void MainWindow::retryArduinoPortDetection()
{
    ...
    const QStringList ports = m_arduino->availablePortNames();
    if (!ports.isEmpty()) {
        // Ports found! Stop the retry timer.
        if (m_arduinoPortDetectionTimer && m_arduinoPortDetectionTimer->isActive()) {
            m_arduinoPortDetectionTimer->stop();
        }
        ...
        // Still no ports. Refresh the list and try again.
        refreshArduinoPortList();
    }
}
...
void MainWindow::ensureArduinoTemperatureConnected()
{
    ...
    if (chosen.isEmpty()) {
        if (m_arduinoTempConnLabel) {
            m_arduinoTempConnLabel->setText(QStringLiteral("Port temperature: aucun port libre."));
        }
        return;
    }
...
QString MainWindow::preferredRfidPortName(const QStringList& ports, bool honorCurrentSelection) const
{
    ...
    if (const QString com4 = chooseIfValid(QStringLiteral("COM4")); !com4.isEmpty()) {
        return com4;
    };

    if (honorCurrentSelection) {
        if (const QString current = chooseIfValid(m_arduinoPortCombo ? m_arduinoPortCombo->currentText() : QString());
            !current.isEmpty()) {
            return current;
        }
    }
    ...
    QSettings settings(QStringLiteral("AquaTech"), QStringLiteral("AquaTech"));
    if (const QString saved = chooseIfValid(settings.value(QStringLiteral("arduino/rfidPortName")).toString());
        !saved.isEmpty()) {
        return saved;
    }

    for (const QString& port : ports) {
        if ((tempPort.isEmpty() || port.compare(tempPort, Qt::CaseInsensitive) != 0)
            && (boatPort.isEmpty() || port.compare(boatPort, Qt::CaseInsensitive) != 0)) {
            return port;
        }
    }
...
QString MainWindow::preferredTemperaturePortName(const QStringList& ports) const
{
    ...
    QSettings settings(QStringLiteral("AquaTech"), QStringLiteral("AquaTech"));
    if (const QString saved = chooseIfValid(settings.value(QStringLiteral("arduino/tempPortName")).toString());
        !saved.isEmpty()) {
        return saved;
    }

    for (const QString& port : ports) {
        if ((rfidPort.isEmpty() || port.compare(rfidPort, Qt::CaseInsensitive) != 0)
            && (boatPort.isEmpty() || port.compare(boatPort, Qt::CaseInsensitive) != 0)) {
            return port;
        }
    }
...
QString MainWindow::preferredBoatPortName(const QStringList& ports) const
{
    ...
    if (const QString com6 = chooseIfValid(QStringLiteral("COM6")); !com6.isEmpty()) {
        return com6;
    }

    QSettings settings(QStringLiteral("AquaTech"), QStringLiteral("AquaTech"));
    if (const QString saved = chooseIfValid(settings.value(QStringLiteral("arduino/boatPortName")).toString());
        !saved.isEmpty()) {
        return saved;
    }

    static const QStringList preferredPorts = {
        QStringLiteral("COM5"),
        QStringLiteral("COM3")
    };
    for (const QString& preferred : preferredPorts) {
        if (const QString chosen = chooseIfValid(preferred); !chosen.isEmpty()) {
            return chosen;
        }
    }
    ...
    for (const QString& port : ports) {
        ...
        if ((port.compare(QStringLiteral("COM4"), Qt::CaseInsensitive) == 0
             || port.compare(QStringLiteral("COM7"), Qt::CaseInsensitive) == 0)
            && ports.size() > 1) {
            continue;
        }
        return port;
