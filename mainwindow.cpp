    ...
    , ui(new Ui::MainWindow)
{
    ...
    if (ui->tableWidgetb) {
        reloadTableWidgetFromDb(ui->tableWidgetb, db, QStringLiteral("bateaux"),
            {QStringLiteral("id"), QStringLiteral("nom"), QStringLiteral("proprietaire"), QStringLiteral("largeur"), QStringLiteral("type"), QStringLiteral("statut"), QStringLiteral("capacite")},
            {});
        ensureActionsColumnPopulated(ui->tableWidgetb, QStringLiteral("QPushButton { border:2px solid rgb(0, 0, 112); border-radius:6px; background-color: rgba(0, 0, 127,0.7); color: white; padding: 8px 16px; font-family: 'Segoe UI Emoji', 'Segoe UI', 'Arial', sans-serif; font-size: 26px; font-weight: bold;} QPushButton:hover { background-color: rgba(0, 0, 127,0.7); border-color: #59abc8;} QPushButton:pressed { background-color:rgba(0, 0, 127,0.9); }"));
        int actionsCol3 = ui->tableWidgetb->columnCount() - 1;
        if (actionsCol3 >= 0) ui->tableWidgetb->setColumnWidth(actionsCol3, 140);
    }
...
void MainWindow::on_pushButton_2b_clicked()
{
    ...
    if (!validateRequiredFields(this, {
            {QStringLiteral("ID Bateau"), ui->lineEdit_3b, [this]{ return !ui->lineEdit_3b || ui->lineEdit_3b->text().trimmed().isEmpty(); }},
            {QStringLiteral("Nom Bateau"), ui->lineEdit_4b, [this]{ return !ui->lineEdit_4b || ui->lineEdit_4b->text().trimmed().isEmpty(); }},
            {QStringLiteral("Propriétaire"), ui->lineEdit_5b, [this]{ return !ui->lineEdit_5b || ui->lineEdit_5b->text().trimmed().isEmpty(); }},
            {QStringLiteral("Largeur"), ui->doubleSpinBox, [this]{ return !ui->doubleSpinBox || ui->doubleSpinBox->value() <= 0.0; }},
            {QStringLiteral("Capacité"), ui->spinBoxb, [this]{ return !ui->spinBoxb || ui->spinBoxb->value() <= 0; }},
        })) {
        return;
    }
    ...
    const QString tableName = resolveTableName(db, {QStringLiteral("BATEAU"), QStringLiteral("BATEAUX"), QStringLiteral("BOAT"), QStringLiteral("T_BATEAU")});
    if (tableName.isEmpty()) {
        QMessageBox::critical(this, QStringLiteral("DB"), QStringLiteral("Table bateaux introuvable (BATEAU/BATEAUX...)."));
        return;
    }

    QHash<QString, QVariant> values;
    values.insert(QStringLiteral("id"), ui->lineEdit_3b ? ui->lineEdit_3b->text().trimmed() : QString());
    values.insert(QStringLiteral("nom"), ui->lineEdit_4b ? ui->lineEdit_4b->text().trimmed() : QString());
    values.insert(QStringLiteral("proprietaire"), ui->lineEdit_5b ? ui->lineEdit_5b->text().trimmed() : QString());
    values.insert(QStringLiteral("largeur"), ui->doubleSpinBox ? ui->doubleSpinBox->value() : 0.0);
    values.insert(QStringLiteral("type"), ui->comboBoxb ? ui->comboBoxb->currentText().trimmed() : QString());
    values.insert(QStringLiteral("statut"), ui->comboBox_2b ? ui->comboBox_2b->currentText().trimmed() : QString());
    values.insert(QStringLiteral("capacite"), ui->spinBoxb ? ui->spinBoxb->value() : 0);
    values.insert(QStringLiteral("dateentree"), ui->dateEdit ? ui->dateEdit->date() : QDate());
    values.insert(QStringLiteral("datedernieremaintenance"), ui->dateEdit_2 ? ui->dateEdit_2->date() : QDate());
    values.insert(QStringLiteral("frequencemaintenance"), ui->spinBox ? ui->spinBox->value() : 0);

    const QHash<QString, QStringList> syn = {
        {QStringLiteral("id"), {QStringLiteral("id"), QStringLiteral("idbateau"), QStringLiteral("boatid"), QStringLiteral("id_bateau")}},
        {QStringLiteral("nom"), {QStringLiteral("nom"), QStringLiteral("nombateau"), QStringLiteral("name")}},
        {QStringLiteral("proprietaire"), {QStringLiteral("proprietaire"), QStringLiteral("owner")}},
        {QStringLiteral("largeur"), {QStringLiteral("largeur"), QStringLiteral("width")}},
        {QStringLiteral("type"), {QStringLiteral("type")}},
        {QStringLiteral("statut"), {QStringLiteral("statut"), QStringLiteral("status")}},
        {QStringLiteral("capacite"), {QStringLiteral("capacite"), QStringLiteral("capacity")}},
        {QStringLiteral("dateentree"), {QStringLiteral("dateentree"), QStringLiteral("date_entree"), QStringLiteral("date")}},
        {QStringLiteral("datedernieremaintenance"), {QStringLiteral("datedernieremaintenance"), QStringLiteral("date_maintenance"), QStringLiteral("lastmaintenance")}},
        {QStringLiteral("frequencemaintenance"), {QStringLiteral("frequencemaintenance"), QStringLiteral("freqmaintenance"), QStringLiteral("maintenancefreq")}},
    };

    QString err;
    if (!insertRowByMapping(this, db, tableName, values, syn, &err)) {
        QMessageBox::critical(this, QStringLiteral("Ajout bateau"), QStringLiteral("Insertion échouée: %1").arg(err));
        return;
    }

    if (ui->tableWidgetb) {
        reloadTableWidgetFromDb(ui->tableWidgetb, db, tableName,
                                {QStringLiteral("id"), QStringLiteral("nom"), QStringLiteral("proprietaire"), QStringLiteral("largeur"), QStringLiteral("type"), QStringLiteral("statut"), QStringLiteral("capacite")},
                                syn);
        ensureActionsColumnPopulated(ui->tableWidgetb, QStringLiteral("QPushButton { border:2px solid rgb(0, 0, 112); border-radius:6px; background-color: rgba(0, 0, 127,0.7); color: white; padding: 8px 16px; font-family: 'Segoe UI Emoji', 'Segoe UI', 'Arial', sans-serif; font-size: 26px; font-weight: bold;} QPushButton:hover { background-color: rgba(0, 0, 127,0.7); border-color: #59abc8;} QPushButton:pressed { background-color:rgba(0, 0, 127,0.9); }") );
        int actionsCol3 = ui->tableWidgetb->columnCount() - 1;
