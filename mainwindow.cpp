...
{
    if (m_arduino || m_arduinoTemp) return;
    ...
    // Remember last selected RFID port.
    QSettings settings(QStringLiteral("AquaTech"), QStringLiteral("AquaTech"));
    const QString savedPort = settings.value(QStringLiteral("arduino/rfidPortName")).toString().trimmed();
    ...
    QTimer::singleShot(350, this, [this]() {
        ...
        const QString portName = preferredRfidPortName(m_arduino->availablePortNames(), true);
        if (portName.isEmpty()) {
            updateArduinoUiState();
            return;
        }
        ...
        if (!m_arduino->connectToPort(portName, 9600, &error)) {
            if (statusBar()) {
                statusBar()->showMessage(QStringLiteral("Arduino auto-connect failed: %1").arg(error), 6000);
            }
            updateArduinoUiState();
        }
    });
...
QString MainWindow::preferredRfidPortName(const QStringList& ports, bool honorCurrentSelection) const
{
    ...
    const auto chooseIfValid = [&](const QString& candidate) -> QString {
        ...
        if (!ports.contains(trimmed)) {
            return QString();
        }
        if (!tempPort.isEmpty() && trimmed.compare(tempPort, Qt::CaseInsensitive) == 0 && ports.size() > 1) {
            return QString();
        }
        return trimmed;
    };
...
QString MainWindow::preferredTemperaturePortName(const QStringList& ports) const
{
    ...
    const auto chooseIfValid = [&](const QString& candidate) -> QString {
        ...
        if (!rfidPort.isEmpty() && trimmed.compare(rfidPort, Qt::CaseInsensitive) == 0 && ports.size() > 1) {
            return QString();
        }
        return trimmed;
    };
