...
private:
    Ui::MainWindow *ui;
    QPlainTextEdit* m_portConsole = nullptr;  // Console de logs pour messages Arduino
    int m_lastKnownDistanceCm = -1;  // Derniere distance calculée
    int m_lastRawPulseMicros = -1;   // Temps d'écho brut Arduino
    int m_currentAssignedQuaiId = -1;  // ID quai assigné
    bool m_boatPresent = false;  // Bateau detecté
    bool m_boatDocked = false;  // Bateau amarré (< 20cm)
    unsigned long m_arrivalDetectedAt = 0;  // Temps detection
    // Baseline calibration for sensor to avoid false positives
    ...
    int m_arduinoReconnectAttempts = 0;
    ...
    // Gestion des messages protocoles Arduino pour les quais
