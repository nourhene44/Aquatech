...
static QString normalizeQuaiStatutForDb(const QString& raw)
{
    const QString s = raw.trimmed();
    if (s.startsWith(QStringLiteral("Occu"), Qt::CaseInsensitive)) {
        return QStringLiteral("Occupe");
    }
    if (s.startsWith(QStringLiteral("Lib"), Qt::CaseInsensitive)) {
        return QStringLiteral("Libre");
    }
    if (s.startsWith(QStringLiteral("Main"), Qt::CaseInsensitive) || s.startsWith(QStringLiteral("Ferm"), Qt::CaseInsensitive)) {
...
bool Quai::ajouter(const QVariantMap &donnees)
{
    ...
    query.addBindValue(normalizeQuaiStatutForDb(donnees.value("STATUT").toString()));
...
bool Quai::modifier(int id, const QVariantMap &donnees)
{
    ...
    for (auto it = donnees.begin(); it != donnees.end(); ++it) {
        ...
        } else if (col.compare(QStringLiteral("STATUT"), Qt::CaseInsensitive) == 0) {
            val = normalizeQuaiStatutForDb(val.toString());
        }
