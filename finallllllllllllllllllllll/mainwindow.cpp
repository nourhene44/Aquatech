...
void MainWindow::setupCapturePageVisuals()
{
    ...
    if (ui->frame_5) {
        ui->frame_5->setGeometry(-190, 75, 340, 715);
    }
    ...
    if (ui->cap_btnValiider_4) {
        ui->cap_btnValiider_4->setGeometry(220, 734, 300, 43);
        ...
        ui->cap_btnValiider_4->setIconSize(QSize(18, 18));
    }
    ...
    if (ui->cap_pagecaptures && ui->frame_zonesb_2) {
        ...
        combo->setGeometry(ui->frame_zonesb_2->width() - 280, 12, 260, 36);
...
void MainWindow::editEmployeFromTable(int row)
{
    ...
    // RÃ©initialiser les champs spÃ©cifiques lors de l'Ã©dition
    m_currentCvPath.clear();
    if (ui->pushButton_5e_2) {
        ui->pushButton_5e_2->setText(QStringLiteral("ðŸ“„ Upload CV"));
...
void MainWindow::ensureEmployeRfidUi()
{
    ...
    if (!rfidStatusLabel && ui->frame_2e) {
        ...
        rfidStatusLabel->setText(QStringLiteral("âš  Arduino: dÃ©connectÃ©"));
...
void MainWindow::on_cap_btnShowTendance_clicked()
{
    ...
    if (auto* idBoatLabel = tendancePage->findChild<QLabel*>(QStringLiteral("cap_label_58"))) {
        idBoatLabel->hide();
    }
    if (auto* idBoatInput = tendancePage->findChild<QLineEdit*>(QStringLiteral("lineEdit_7c_3"))) {
        idBoatInput->hide();
        idBoatInput->clear();
    }
    if (auto* idBoatLabel2 = tendancePage->findChild<QLabel*>(QStringLiteral("cap_label_idBateau"))) {
        idBoatLabel2->hide();
    }
    if (auto* idBoatValue = tendancePage->findChild<QLineEdit*>(QStringLiteral("lineEdit_7c_12"))) {
        idBoatValue->hide();
        idBoatValue->clear();
    }
...
void MainWindow::on_cap_pushButton_4_clicked()
{
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        if (auto* point = tendancePage->findChild<QLabel*>(QStringLiteral("p%1").arg(i + 1))) {
            ...
            const int barLines = qBound(1, 1 + static_cast<int>(ratio * 8.0 + 0.5), 9);
            QStringList bars;
            for (int j = 0; j < barLines; ++j) {
                bars << QStringLiteral("#");
            }
            point->setText(bars.join(QStringLiteral("\n")));
            ...
            const QString color = extractColor(point->styleSheet());
            point->setStyleSheet(QStringLiteral("color:%1; font-size:14px;").arg(color));
        }
    }
}
...
void MainWindow::updateCapturesTemperatureSummaryUi()
{
    ...
    if (ui && ui->cap_lineEdit_8) {
        ...
        ui->cap_lineEdit_8->setStyleSheet(
            QStringLiteral("QLineEdit { background-color: rgb(236, 241, 248); border: 2px solid %1; border-radius: 10px; color: %2; font-size: 14px; font-weight: 700; padding: 6px 10px; }")
...
void MainWindow::updateEmployeePageRfidStatus()
{
    ...
    if (connected) {
        ...
        rfidStatusLabel->setText(QStringLiteral("âœ“ Arduino: %1").arg(portName));
...
static void updateCapturesStats(MainWindow* parent, Ui::MainWindow* ui)
{
    ...
    const QStringList accentColors = {
        ...
        QStringLiteral("#f39c12"),
        QStringLiteral("#c51414"),
        QStringLiteral("#9b59b6")
    };
    ...
    if (all.isEmpty() || totalAll <= 0.0) {
        for (int i = 0; i < 5; ++i) {
            ...
            if (valueLabel) {
                ...
                valueLabel->setStyleSheet(valueBoxStyle(accentColors.value(i, QStringLiteral("#9b59b6"))));
            }
        }
        return;
    }
    ...
    for (int i = 0; i < 5; ++i) {
        // RÃ©cupÃ©rer les widgets pour cette colonne
        auto* label = ui->frame_zonesb_2->findChild<QLabel*>(labelNames[i]);
        auto* progressBar = ui->frame_zonesb_2->findChild<QProgressBar*>(progressNames[i]);
        auto* valueLabel = ui->frame_zonesb_2->findChild<QLabel*>(valueNames[i]);
