...
QString findQuotaTable(QSqlDatabase db)
{
	const QStringList tables = db.tables(QSql::Tables) + db.tables(QSql::Views);
	const QStringList candidates = {
...
QStringList getColumnNames(QSqlDatabase db, const QString& table)
{
	QStringList cols;
	if (!db.isValid() || !db.isOpen() || table.trimmed().isEmpty()) {
		return cols;
	}
...
bool resolveQuotaColumns(QSqlDatabase db,
					QString* quotaTableOut,
					QString* typeColOut,
					QString* quotaColOut,
					QString* weekStartColOut,
					QString* weekEndColOut)
{
	if (!quotaTableOut || !typeColOut || !quotaColOut) {
		s_lastError = QStringLiteral("Erreur interne (parametres quota).");
		return false;
	}

	const QString quotaTable = findQuotaTable(db);
	if (quotaTable.isEmpty()) {
		s_lastError = QStringLiteral("Table QUOTA introuvable dans la base.");
		return false;
	}
	...
	const QString weekStartCol = matchColumnBySynonyms(cols, {
		QStringLiteral("date_debut"), QStringLiteral("debut"), QStringLiteral("start_date"),
		QStringLiteral("week_start"), QStringLiteral("semaine_debut"), QStringLiteral("datedebut")
	});
	const QString weekEndCol = matchColumnBySynonyms(cols, {
		QStringLiteral("date_fin"), QStringLiteral("fin"), QStringLiteral("end_date"),
		QStringLiteral("week_end"), QStringLiteral("semaine_fin"), QStringLiteral("datefin")
	});

	if (typeCol.isEmpty() || quotaCol.isEmpty()) {
		s_lastError = QStringLiteral("Colonnes QUOTA manquantes (type + valeur).");
		return false;
	}

	*quotaTableOut = quotaTable;
	*typeColOut = typeCol;
	*quotaColOut = quotaCol;
	if (weekStartColOut) *weekStartColOut = weekStartCol;
	if (weekEndColOut) *weekEndColOut = weekEndCol;
	return true;
}

bool fetchQuotaKgForSpecies(QSqlDatabase db,
...
{
	if (outKg) *outKg = 0.0;
	if (species.trimmed().isEmpty()) {
		s_lastError = QStringLiteral("Type poisson obligatoire pour verifier le quota.");
		return false;
	}
	...
	QString typeCol;
	QString quotaCol;
	QString weekStartCol;
	QString weekEndCol;
	if (!resolveQuotaColumns(db, &quotaTable, &typeCol, &quotaCol, &weekStartCol, &weekEndCol)) {
		return false;
	}

	const bool hasWeekCols = !weekStartCol.isEmpty() && !weekEndCol.isEmpty();
	const bool weekValid = weekStart.isValid() && weekEnd.isValid() && weekEnd >= weekStart;

	// En cas de doublons dans QUOTA (ex: 'Thon' et 'THON'), prendre la valeur la plus stricte.
	QSqlQuery q(db);
	if (hasWeekCols && weekValid) {
		q.prepare(QStringLiteral(
			"SELECT MIN(%1) "
			"FROM %2 "
			"WHERE UPPER(TRIM(%3)) = UPPER(TRIM(?)) "
			"  AND TRUNC(%4) = TO_DATE(?, 'YYYY-MM-DD') "
			"  AND TRUNC(%5) = TO_DATE(?, 'YYYY-MM-DD')")
				  .arg(quotaCol, quotaTable, typeCol, weekStartCol, weekEndCol));
		q.addBindValue(species.trimmed());
		q.addBindValue(weekStart.toString(QStringLiteral("yyyy-MM-dd")));
		q.addBindValue(weekEnd.toString(QStringLiteral("yyyy-MM-dd")));
	} else {
		q.prepare(QStringLiteral("SELECT MIN(%1) FROM %2 WHERE UPPER(TRIM(%3)) = UPPER(TRIM(?))")
				  .arg(quotaCol, quotaTable, typeCol));
		q.addBindValue(species.trimmed());
	}

	if (!q.exec()) {
		s_lastError = friendlyOracleError(q.lastError().text());
		return false;
	}
	if (!q.next() || q.value(0).isNull()) {
		if (hasWeekCols && weekValid) {
			s_lastError = QStringLiteral("Quota introuvable pour l'espece: %1 (semaine %2..%3)")
					 .arg(species.trimmed(),
						  weekStart.toString(QStringLiteral("yyyy-MM-dd")),
						  weekEnd.toString(QStringLiteral("yyyy-MM-dd")));
		} else {
			s_lastError = QStringLiteral("Quota introuvable pour l'espece: %1").arg(species.trimmed());
		}
		return false;
	}

	bool ok = false;
	const double kg = q.value(0).toDouble(&ok);
	if (!ok || kg < 0.0) {
		s_lastError = QStringLiteral("Valeur quota invalide pour l'espece: %1").arg(species.trimmed());
		return false;
	}
...
bool checkWeeklyQuotaNotExceeded(QSqlDatabase db,
						 const QString& capturesTable,
						 const QString& idCol,
						 const QString& idBateauCol,
						 const QString& typeCol,
						 const QString& poidsCol,
						 const QString& dateCol,
						 int idBateau,
						 const QString& species,
						 double newPoidsKg,
						 const QDate& captureDate,
						 const QVariant& excludeCaptureId)
{
	if (idBateau <= 0) {
	...
	const QDate effectiveDate = captureDate.isValid() ? captureDate : QDate::currentDate();
	const QDate weekStart = effectiveDate.addDays(1 - effectiveDate.dayOfWeek());
	const QDate weekEnd = weekStart.addDays(6);

	double quotaKg = 0.0;
	if (!fetchQuotaKgForSpecies(db, species, weekStart, weekEnd, &quotaKg)) {
		return false;
	}
	...
	QString sql = QStringLiteral(
		...
		"FROM %2 "
		"WHERE %3 = ? "
		"  AND UPPER(TRIM(%4)) = UPPER(TRIM(?)) "
		"  AND TRUNC(%5) BETWEEN TO_DATE(?, 'YYYY-MM-DD') AND TO_DATE(?, 'YYYY-MM-DD')")
					 .arg(poidsCol, capturesTable, idBateauCol, typeCol, dateCol);

	QList<QVariant> binds;
	binds << idBateau;
	binds << species.trimmed();
	binds << weekStart.toString(QStringLiteral("yyyy-MM-dd"));
	binds << weekEnd.toString(QStringLiteral("yyyy-MM-dd"));

	if (excludeCaptureId.isValid() && !excludeCaptureId.isNull()) {
		sql += QStringLiteral(" AND %1 <> ?").arg(idCol);
		binds << excludeCaptureId;
	}
	...
	double currentWeekKg = 0.0;
	if (q.next()) {
		currentWeekKg = q.value(0).toDouble();
	}

	const double projectedKg = currentWeekKg + newPoidsKg;
	const double eps = 1e-9;
	if (projectedKg > quotaKg + eps) {
		s_lastError = QStringLiteral(
			"Surpeche detectee (quota hebdomadaire depasse). "
			"Bateau=%1, Espece=%2, Semaine=%3..%4, "
			"Deja=%5kg, Nouveau=%6kg, Quota=%7kg")
					 .arg(QString::number(idBateau),
						 species.trimmed(),
						 weekStart.toString(QStringLiteral("yyyy-MM-dd")),
						 weekEnd.toString(QStringLiteral("yyyy-MM-dd")),
						 QString::number(currentWeekKg, 'f', 2),
						 QString::number(newPoidsKg, 'f', 2),
						 QString::number(quotaKg, 'f', 2));
		return false;
	}
...
bool captures::ajouter() const
{
	...
	if (dateCapture_.isValid() && dateCapture_ > QDate::currentDate()) {
		s_lastError = QStringLiteral("Date capture invalide (ne doit pas etre superieure a aujourd'hui).");
		return false;
	}
	...
	// Controle surpeche: ne pas depasser le quota hebdomadaire (table QUOTA) pour ce bateau + espece.
	const QDate effectiveDate = dateCapture_.isValid() ? dateCapture_ : QDate::currentDate();
	if (!checkWeeklyQuotaNotExceeded(db,
								 tableName,
								 idCol,
								 idBateauCol,
								 typeCol,
								 ...
								 effectiveDate,
								 QVariant())) {
		// Demande: l'ajout doit passer meme si le quota est depasse.
		// On n'ignore que le cas "Surpeche detectee"; les autres erreurs (DB/quota introuvable)
		// restent bloquantes.
		const QString err = s_lastError;
		if (err.startsWith(QStringLiteral("Surpeche detectee"))) {
			s_lastError.clear();
		} else {
			return false;
		}
...
bool captures::modifierAvecAncienId(const QString& ancienId) const
{
	...
	// Controle surpeche: recalculer sur la semaine de la nouvelle date, en excluant la capture courante.
	const QDate effectiveDate = dateCapture_.isValid() ? dateCapture_ : QDate::currentDate();
	if (!checkWeeklyQuotaNotExceeded(db,
								 tableName,
								 idCol,
...
bool captures::verifierSurpeche(const QString& idCapture)
{
	if (idCapture.trimmed().isEmpty()) {
		s_lastError = QStringLiteral("ID capture invalide.");
		return false;
	...
	// Support multi-lignes: essayer ID direct, sinon intervalle baseId*10..baseId*10+9
	QVector<int> candidateIds;
	candidateIds.push_back(idCaptureNum);
	for (int i = 0; i < 10; ++i) {
		candidateIds.push_back(idCaptureNum * 10 + i);
	}

	bool foundAny = false;
	for (int candId : candidateIds) {
		QSqlQuery q(db);
		q.prepare(QStringLiteral("SELECT %1, %2, %3, %4 FROM %5 WHERE %6 = ?")
				  .arg(typeCol, poidsCol, dateCol, idBateauCol, tableName, idCol));
		q.addBindValue(candId);
		if (!q.exec()) {
			s_lastError = friendlyOracleError(q.lastError().text());
			return false;
		}
		...
		foundAny = true;
		do {
			const QString species = q.value(0).toString();
			const double poidsKg = q.value(1).toDouble();
			const QDate d = q.value(2).toDate();
			const int idBateauFromDb = q.value(3).toInt();
			if (idBateauFromDb <= 0) {
				s_lastError = QStringLiteral("ID bateau invalide pour la capture ID=%1.")
						 .arg(QString::number(candId));
				return false;
			}
	...
	if (!foundAny) {
		s_lastError = QStringLiteral("Capture introuvable pour ID=%1.")
