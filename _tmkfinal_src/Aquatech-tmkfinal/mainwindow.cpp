...
static QVector<CaptureLineUi> captureLinesFromTable(QTableWidget* table)
{
    ...
    for (int r = 0; r < table->rowCount(); ++r) {
        CaptureLineUi l;
        if (auto* it0 = table->item(r, 0)) l.species = it0->text().trimmed();
        if (auto* it1 = table->item(r, 1)) l.quantite = toIntOrZero(it1->text());
        if (auto* it2 = table->item(r, 2)) l.poidsKg = it2->text().trimmed().toDouble();

        if (l.species.isEmpty()) continue;
        if (l.quantite <= 0) continue;
        if (l.poidsKg <= 0.0) continue;
        lines.push_back(l);
    }
    return lines;
}
...
static bool hasSpeciesInLinesTable(QTableWidget* table, const QString& species)
{
    ...
    for (int r = 0; r < table->rowCount(); ++r) {
        ...
        if (!it) continue;
        if (normalizeKey(it->text()) == key) return true;
    }
    return false;
}
...
static int captureDisplayIdFromStoredId(int storedId)
{
    if (storedId <= 0) return storedId;
    // Multi-ligne: on stocke en base ID = baseId*10 + i (8 chiffres). Affichage = baseId.
    // Mono-ligne: ID reste baseId (7 chiffres) => affichage identique.
    if (storedId >= 10000000) {
        return storedId / 10;
    }
    return storedId;
}

static QStringList captureStoredIdsFromDisplayId(int displayId)
{
    QStringList ids;
    if (displayId <= 0) return ids;
    // Par convention: une capture multi-espèces peut être stockée sous displayId*10 .. displayId*10+9
    for (int i = 0; i < 10; ++i) {
        ids << QString::number(displayId * 10 + i);
    }
    // Le cas mono-ligne: la capture peut aussi être stockée directement sous displayId
    ids.prepend(QString::number(displayId));
    return ids;
}
    ...
    , m_editingCaptureId()
{
    ...
    // Captures: la date d'ajout ne doit pas depasser aujourd'hui.
    if (ui->cap_deDateCapture_3) {
        ui->cap_deDateCapture_3->setMaximumDate(today);
        if (!ui->cap_deDateCapture_3->date().isValid() || ui->cap_deDateCapture_3->date() > today) {
            ui->cap_deDateCapture_3->setDate(today);
        }
    }

    // Captures: initialiser la table des lignes multi-espèces
    if (ui->cap_tableWidget_Lignes) {
        ui->cap_tableWidget_Lignes->setEditTriggers(QAbstractItemView::NoEditTriggers);
        ui->cap_tableWidget_Lignes->setSelectionBehavior(QAbstractItemView::SelectRows);
        ui->cap_tableWidget_Lignes->setSelectionMode(QAbstractItemView::SingleSelection);
        ui->cap_tableWidget_Lignes->horizontalHeader()->setStretchLastSection(true);
        ui->cap_tableWidget_Lignes->verticalHeader()->setVisible(false);
        ui->cap_tableWidget_Lignes->setShowGrid(true);
        ui->cap_tableWidget_Lignes->setColumnWidth(0, 110);
        ui->cap_tableWidget_Lignes->setColumnWidth(1, 50);
        ui->cap_tableWidget_Lignes->setColumnWidth(2, 70);
    }
...
bool MainWindow::saveQuotas(QString* errorOut)
{
    ...
    qExists.prepare(QStringLiteral("SELECT 1 FROM %1 WHERE UPPER(TRIM(%2)) = UPPER(TRIM(?))").arg(quotaTable, typeCol));

    QSqlQuery qUpdate(db);
    if (!updatedAtCol.isEmpty()) {
        qUpdate.prepare(QStringLiteral("UPDATE %1 SET %2 = ?, %3 = SYSDATE WHERE UPPER(TRIM(%4)) = UPPER(TRIM(?))")
                            .arg(quotaTable, quotaCol, updatedAtCol, typeCol));
    } else {
        qUpdate.prepare(QStringLiteral("UPDATE %1 SET %2 = ? WHERE UPPER(TRIM(%3)) = UPPER(TRIM(?))").arg(quotaTable, quotaCol, typeCol));
    }
...
void MainWindow::loadCaptures()
{
    ...
    auto groupKey = [](const QString& groupId, int bateau, const QDate& date) {
        return QStringLiteral("%1|%2|%3")
            .arg(groupId, QString::number(bateau), date.toString(QStringLiteral("yyyy-MM-dd")));
    };

    // Détection robuste des captures multi-espèces :
    // On génère en base ID_CAPTURE = baseId*10 + i (i=0..)
    // Pour éviter d'interpréter à tort un ID "normal" comme multi-espèces, on ne regroupe
    // que si on observe au moins 2 lignes partageant le même (storedId/10) ET qu'il existe
    // une ligne avec suffixe 0 dans ce groupe (celle générée en premier).
    QHash<QString, BaseGroupInfo> baseInfos;
    auto baseKey = [](int baseId, int bateau, const QDate& date) {
        return QStringLiteral("%1|%2|%3")
            .arg(QString::number(baseId), QString::number(bateau), date.toString(QStringLiteral("yyyy-MM-dd")));
    };

    for (const captures::TableRowData& r : rows) {
        const int storedId = toIntOrZero(r.idCapture);
        if (storedId <= 0) continue;
        const int baseId = storedId / 10;
        if (baseId <= 0) continue;
        const int suffix = storedId % 10;

        BaseGroupInfo& info = baseInfos[baseKey(baseId, r.idBateau, r.dateCapture)];
        ++info.count;
        if (suffix == 0) info.hasSuffix0 = true;
    }
    ...
    for (const captures::TableRowData& r : rows) {
        ...
        if (storedId > 0) {
            baseId = storedId / 10;
            const int suffix = storedId % 10;
            const BaseGroupInfo info = baseInfos.value(baseKey(baseId, r.idBateau, r.dateCapture));
            isMultiRow = (baseId > 0 && suffix >= 0 && suffix <= 9 && info.count >= 2 && info.hasSuffix0);
            if (isMultiRow) {
                displayId = baseId;
            }
        }

        const QString groupId = isMultiRow
            ? QStringLiteral("M:%1").arg(QString::number(baseId))
            : QStringLiteral("S:%1").arg(storedId > 0 ? QString::number(storedId) : r.idCapture);

        const QString key = groupKey(groupId, r.idBateau, r.dateCapture);
        Group& g = grouped[key];
        g.displayId = displayId;
        g.bateau = r.idBateau;
        g.date = r.dateCapture;
        g.storedIds << (storedId > 0 ? QString::number(storedId) : r.idCapture);
		Group::Line line;
		line.species = r.typePoisson;
		line.quantite = r.quantite;
		line.poidsKg = r.poids;
    ...
    for (auto it = grouped.constBegin(); it != grouped.constEnd(); ++it) {
        ...
        const bool isGroupedRow = (g.storedIds.size() > 1);

        // Agr├⌐gation par type (au cas o├╣ plusieurs lignes auraient le m├¬me type)
        struct Agg {
            QString species;
            int quantite = 0;
            double poidsKg = 0.0;
        };
        QHash<QString, Agg> bySpecies;
        int totalQuantite = 0;
        double totalPoids = 0.0;
        for (const auto& l : g.lines) {
            const QString key = normalizeKey(l.species);
            Agg a = bySpecies.value(key);
            if (a.species.isEmpty()) a.species = l.species.trimmed();
            a.quantite += l.quantite;
            a.poidsKg += l.poidsKg;
            bySpecies.insert(key, a);
            totalQuantite += l.quantite;
            totalPoids += l.poidsKg;
        }
        const int distinctTypes = bySpecies.size();

        auto* idItem = new QTableWidgetItem(QString::number(g.displayId));
        idItem->setData(Qt::UserRole, g.storedIds.join(QStringLiteral(",")));
        if (isGroupedRow) {
            idItem->setTextAlignment(Qt::AlignLeft | Qt::AlignTop);
        }
        table->setItem(row, 0, idItem);

        auto* bateauItem = new QTableWidgetItem(QString::number(g.bateau));
        if (isGroupedRow) {
            bateauItem->setTextAlignment(Qt::AlignLeft | Qt::AlignTop);
        }
        table->setItem(row, 1, bateauItem);
        auto* speciesItem = new QTableWidgetItem(QString::number(distinctTypes));
        speciesItem->setTextAlignment(Qt::AlignLeft | Qt::AlignVCenter);
        // Pour le mode modification (mono-ligne), garder la vraie valeur en UserRole.
        if (g.lines.size() == 1) {
            speciesItem->setData(Qt::UserRole, g.lines.at(0).species.trimmed());
        } else {
            QStringList raw;
            raw.reserve(g.lines.size());
            for (const auto& l : g.lines) raw << l.species.trimmed();
            speciesItem->setData(Qt::UserRole, raw.join(QStringLiteral("\n")));
        }
        table->setItem(row, 2, speciesItem);

        auto* qtyItem = new QTableWidgetItem(QString::number(totalQuantite));
        qtyItem->setTextAlignment(Qt::AlignLeft | Qt::AlignVCenter);
        qtyItem->setData(Qt::UserRole, QString::number(totalQuantite));
        table->setItem(row, 3, qtyItem);

        auto* poidsItem = new QTableWidgetItem(QString::number(totalPoids, 'f', 2));
        poidsItem->setTextAlignment(Qt::AlignLeft | Qt::AlignVCenter);
        poidsItem->setData(Qt::UserRole, QString::number(totalPoids, 'f', 2));
        table->setItem(row, 4, poidsItem);
        auto* dateItem = new QTableWidgetItem(g.date.isValid() ? g.date.toString(QStringLiteral("yyyy-MM-dd")) : QString());
        if (isGroupedRow) {
            dateItem->setTextAlignment(Qt::AlignLeft | Qt::AlignTop);
        }
        table->setItem(row, 5, dateItem);

		// Lignes agr├⌐g├⌐es: hauteur standard
		table->setRowHeight(row, 42);
        ...
        if (isGroupedRow) {
            ...
            btnEdit->setToolTip(QStringLiteral("Modification non disponible pour une capture multi-especes"));
        } else {
        ...
        connect(btnView, &QPushButton::clicked, this, [this, displayId = g.displayId, bySpecies]() {
            QDialog dlg(this);
            dlg.setWindowTitle(QStringLiteral("Details capture %1").arg(QString::number(displayId)));
            dlg.setModal(true);

            auto* layout = new QVBoxLayout(&dlg);
            auto* t = new QTableWidget(&dlg);
            t->setColumnCount(3);
            t->setHorizontalHeaderItem(0, new QTableWidgetItem(QStringLiteral("Nb Type de Poisson")));
            t->setHorizontalHeaderItem(1, new QTableWidgetItem(QStringLiteral("Qte Totale")));
            t->setHorizontalHeaderItem(2, new QTableWidgetItem(QStringLiteral("Poids Totale")));
            ...
            QList<Agg> list = bySpecies.values();
            std::sort(list.begin(), list.end(), [](const Agg& a, const Agg& b) {
                return a.species.toLower() < b.species.toLower();
            });
            t->setRowCount(list.size());
            for (int i = 0; i < list.size(); ++i) {
                const Agg& a = list.at(i);
                t->setItem(i, 0, new QTableWidgetItem(a.species));
                t->setItem(i, 1, new QTableWidgetItem(QString::number(a.quantite)));
                t->setItem(i, 2, new QTableWidgetItem(QString::number(a.poidsKg, 'f', 2)));
            }
...
void MainWindow::loadCaptureFromTable(int row)
{
    ...
    // En mode modification, on laisse le formulaire en mono-espèce.
    if (ui->cap_tableWidget_Lignes) {
...
void MainWindow::supprimerCaptureFromRow(int row)
{
    ...
    if (auto* idItem = table->item(row, 0)) {
        const QString data = idItem->data(Qt::UserRole).toString().trimmed();
        if (!data.isEmpty()) {
            storedIds = data.split(QLatin1Char(','), Qt::SkipEmptyParts);
            for (QString& s : storedIds) s = s.trimmed();
        }
    }
    ...
    const bool isGrouped = storedIds.size() > 1;
    ...
    // Supprimer toutes les lignes sous-jacentes
    for (const QString& id : storedIds) {
        if (!captures::supprimer(id)) {
...
void MainWindow::on_cap_btnAddCaptureLine_clicked()
{
    ...
    if (!m_editingCaptureId.isEmpty()) {
        QMessageBox::information(this, QStringLiteral("Capture"), QStringLiteral("L'ajout multi-espèces est disponible uniquement en mode ajout."));
        return;
    }
    ...
    if (hasSpeciesInLinesTable(ui->cap_tableWidget_Lignes, species)) {
        QMessageBox::warning(this, QStringLiteral("Doublon"), QStringLiteral("Cette espece est deja ajoutee. Retirez-la puis re-ajoutez si besoin."));
        return;
    }
...
void MainWindow::on_cap_btnRemoveCaptureLine_clicked()
{
    ...
    QTableWidget* t = ui->cap_tableWidget_Lignes;
    int row = -1;
    if (t->currentRow() >= 0) {
        row = t->currentRow();
...
void MainWindow::on_cap_btnValiider_3_clicked()
{
    ...
    if (!hasLinesTable && !hasCurrentLine) {
        QMessageBox::warning(this, QStringLiteral("Champs"), QStringLiteral("Ajoutez au moins une espece (bouton ➕ Ajouter) ou saisissez Quantite+Poids."));
        return;
    }
    ...
    if (ui->cap_deDateCapture_3) {
        ...
        if (d.isValid() && d > QDate::currentDate()) {
            QMessageBox::warning(this,
                                 ...
                                 QStringLiteral("La date de capture ne doit pas etre superieure a aujourd'hui."));
            return;
        }
    }
    ...
    if (!m_editingCaptureId.isEmpty()) {
        ...
        // Si l'utilisateur n'a pas cliqué sur ➕ Ajouter, mais a rempli la ligne courante, on l'inclut.
        ...
        QString baseId = ui->cap_lineEdit_11 ? ui->cap_lineEdit_11->text().trimmed() : QString();
        if (baseId.isEmpty()) {
            baseId = captures::genererNouvelId();
            if (ui->cap_lineEdit_11 && !baseId.isEmpty()) ui->cap_lineEdit_11->setText(baseId);
        }
        ...
        if (lines.size() == 1) {
            ...
            // Multi-lignes: stocker en base sous forme baseId*10 + i pour regrouper a l'affichage
            ...
            for (const QString& id : ids) {
                if (captures::idExiste(id)) {
                    if (canTx) db.rollback();
                    QMessageBox::warning(this, QStringLiteral("Doublon"), QStringLiteral("Un des IDs genere existe deja. Choisissez un autre ID Capture."));
                    return;
                }
            }
        }
        ...
        const QDate d = (ui->cap_deDateCapture_3 && ui->cap_deDateCapture_3->date().isValid())
            ? ui->cap_deDateCapture_3->date()
            : QDate::currentDate();

        for (int i = 0; i < lines.size(); ++i) {
            const CaptureLineUi& l = lines[i];
            captures one(ids[i], idBateau, l.species, l.quantite, l.poidsKg, d);
            if (!one.ajouter()) {
                const QString err = captures::lastError();
                if (canTx) db.rollback();
                if (isSaisieConstraintError(err)) {
                                        ...
                                        QString* errorOut)
{
    ...
    const QStringList capCols = getColumnNames(db, capturesTable);
    const QString idCol = matchColumnBySynonyms(capCols, {
        QStringLiteral("id_capture"), QStringLiteral("idcapture"), QStringLiteral("id")
    });
    const QString idBateauCol = matchColumnBySynonyms(capCols, {
        QStringLiteral("id_bateau"), QStringLiteral("idbateau"), QStringLiteral("bateau_id")
    });
    const QString typeCol = matchColumnBySynonyms(capCols, {
        QStringLiteral("type_poisson"), QStringLiteral("typepoisson"), QStringLiteral("poisson"), QStringLiteral("espece")
    });
    const QString quantiteCol = matchColumnBySynonyms(capCols, {
        QStringLiteral("quantite"), QStringLiteral("qte")
    });
    const QString poidsCol = matchColumnBySynonyms(capCols, {
        QStringLiteral("poids"), QStringLiteral("poids_kg")
    });
    const QString dateCol = matchColumnBySynonyms(capCols, {
    ...
    // Lire la capture pour obtenir la date (base de la semaine)
    int idBateau = 0;
    QDate captureDate;
    {
        QSqlQuery q(db);
        // Support multi-lignes: si l'ID affiche (baseId) n'existe pas tel quel,
        // on tente sur l'intervalle baseId*10..baseId*10+9.
        ...
        q.addBindValue(idCaptureNum);
        q.addBindValue(idCaptureNum * 10);
        q.addBindValue(idCaptureNum * 10 + 9);
        if (!q.exec()) {
            if (errorOut) *errorOut = QStringLiteral("Lecture capture echouee: %1").arg(q.lastError().text());
            return false;
        }
        if (!q.next()) {
            if (errorOut) *errorOut = QStringLiteral("Capture introuvable pour ID=%1.")
                            .arg(QString::number(idCaptureNum));
            return false;
        }
        captureDate = q.value(0).toDate();
        idBateau = q.value(1).toInt();
        if (idBateau <= 0) {
            if (errorOut) *errorOut = QStringLiteral("ID Bateau invalide pour la capture ID=%1.")
                            .arg(QString::number(idCaptureNum));
            return false;
        }
        if (!captureDate.isValid()) {
            // fallback: si la date est invalide, on reste quand meme coherent
    ...
    // Charger les quotas en memoire (type -> quota kg)
    QHash<QString, double> quotaBySpecies;
    {
        const QString quotaTable = resolveTableName(db, {
            QStringLiteral("QUOTA"),
        ...
        const QStringList qCols = getColumnNames(db, quotaTable);
        const QString quotaTypeCol = matchColumnBySynonyms(qCols, {
            QStringLiteral("type_poisson"), QStringLiteral("espece"), QStringLiteral("poisson"),
            QStringLiteral("species"), QStringLiteral("type")
        });
        const QString quotaKgCol = matchColumnBySynonyms(qCols, {
            QStringLiteral("quota"), QStringLiteral("quota_kg"), QStringLiteral("quotaKg"),
            QStringLiteral("poids_max"), QStringLiteral("max_poids"), QStringLiteral("limite")
        });
        ...
        QSqlQuery q(db);
        if (!q.exec(QStringLiteral("SELECT %1, %2 FROM %3").arg(quotaTypeCol, quotaKgCol, quotaTable))) {
            if (errorOut) *errorOut = QStringLiteral("Lecture quotas echouee: %1").arg(q.lastError().text());
            return false;
        }
        while (q.next()) {
            const QString species = q.value(0).toString().trimmed();
            bool ok = false;
            const double kg = q.value(1).toDouble(&ok);
            if (!species.isEmpty() && ok && kg >= 0.0) {
                const QString key = normalizeKey(species);
                const auto it = quotaBySpecies.constFind(key);
                if (it == quotaBySpecies.constEnd()) {
    ...
    QVector<Row> rows;
    {
        ...
        q.addBindValue(idBateau);
        q.addBindValue(weekStart.toString(QStringLiteral("yyyy-MM-dd")));
        q.addBindValue(weekEnd.toString(QStringLiteral("yyyy-MM-dd")));
        if (!q.exec()) {
            if (errorOut) *errorOut = QStringLiteral("Analyse captures echouee: %1").arg(q.lastError().text());
            return false;
        }
        while (q.next()) {
            Row r;
            r.species = q.value(0).toString().trimmed();
            r.quantite = q.value(1).toInt();
            r.poidsKg = q.value(2).toDouble();
            if (r.species.isEmpty()) continue;

            const auto it = quotaBySpecies.constFind(normalizeKey(r.species));
            if (it != quotaBySpecies.constEnd()) {
                r.quotaKg = *it;
            }
            if (r.quotaKg > 0.0) {
                r.pct = (r.poidsKg >= 0.0) ? (100.0 * r.poidsKg / r.quotaKg) : 0.0;
            } else {
                r.pct = (r.poidsKg > 0.0) ? 999.0 : 0.0;
            }
            if (r.quotaKg == 0.0 && r.poidsKg > 0.0) {
                r.etat = QStringLiteral("Surpeche");
            } else if (r.quotaKg > 0.0 && r.pct > 100.0) {
                r.etat = QStringLiteral("Surpeche");
            } else if (r.quotaKg > 0.0 && r.pct >= 80.0) {
                r.etat = QStringLiteral("Alerte");
            } else {
                r.etat = QStringLiteral("OK");
            }
            rows.push_back(r);
        }
    }

    // Remplir la page surpeche
    ...
    if (ui->lineEdit_7c_9) ui->lineEdit_7c_9->setText(QString::number(rows.size()));

    bool anySurpeche = false;
    QStringList surpecheSpecies;
    for (const Row& r : rows) {
        if (r.etat == QStringLiteral("Surpeche")) {
            anySurpeche = true;
            surpecheSpecies << r.species;
        }
    }
    ...
    if (ui->cap_tableWidget_4) {
        QTableWidget* t = ui->cap_tableWidget_4;
        t->clearContents();
        t->setRowCount(rows.size());
        for (int i = 0; i < rows.size(); ++i) {
            const Row& r = rows[i];
            t->setItem(i, 0, new QTableWidgetItem(r.species));
            t->setItem(i, 1, new QTableWidgetItem(QString::number(r.quantite)));
            t->setItem(i, 2, new QTableWidgetItem(QString::number(r.poidsKg, 'f', 2) + QStringLiteral("kg")));
            if (r.quotaKg >= 0.0) {
                t->setItem(i, 3, new QTableWidgetItem(QString::number(r.quotaKg, 'f', 0) + QStringLiteral("kg")));
            } else {
                t->setItem(i, 3, new QTableWidgetItem(QStringLiteral("N/A")));
            }
            const double pctShown = qMin(999.0, qMax(0.0, r.pct));
            t->setItem(i, 4, new QTableWidgetItem(QString::number(pctShown, 'f', 1) + QStringLiteral("%")));
            t->setItem(i, 5, new QTableWidgetItem(r.etat));
        }
    }

    // Indicateurs (top 3)
    QVector<Row> top = rows;
    ...
    for (int i = 0; i < qMin(3, top.size()); ++i) {
        const Row& r = top[i];
        const int val = qBound(0, static_cast<int>(qMin(100.0, qMax(0.0, r.pct)) + 0.5), 100);
        if (i == 0) {
            if (ui->cap_label_24) ui->cap_label_24->setText(QStringLiteral("Nom espece : %1").arg(r.species));
            if (ui->cap_progressBar_4) ui->cap_progressBar_4->setValue(val);
        } else if (i == 1) {
            if (ui->cap_label_25) ui->cap_label_25->setText(QStringLiteral("Nom espece : %1").arg(r.species));
            if (ui->cap_progressBar_5) ui->cap_progressBar_5->setValue(val);
        } else if (i == 2) {
            if (ui->cap_label_26) ui->cap_label_26->setText(QStringLiteral("Nom espece: %1").arg(r.species));
            if (ui->cap_progressBar_6) ui->cap_progressBar_6->setValue(val);
        }
    }
    ...
    if (ui->cap_textBrowser_2) {
        ...
        if (rows.isEmpty()) {
            ...
            html += QStringLiteral("<p><b>Surpeche detectee</b> sur: %1</p>").arg(surpecheSpecies.join(QStringLiteral(", ")));
        } else {
...
void MainWindow::on_cap_btnShowSurpeche_clicked()
{
    ...
    QString err;
    if (!loadSurpecheAnalysisForCapture(this, ui, idCaptureText, &err)) {
        QMessageBox::warning(this, QStringLiteral("Surpeche"), err.isEmpty() ? QStringLiteral("Verification surpeche echouee.") : err);
    }
...
void MainWindow::on_cap_btnShowQuotaWeek_clicked()
{
    // Compat: ce slot existait dans d'anciennes versions de l'UI.
    // Il affiche la page Surpeche et calcule les quotas de la semaine.
    on_cap_btnShowSurpeche_clicked();
}

void MainWindow::on_cap_btnRefreshSurpeche_clicked()
{
    if (!ui) {
        return;
    }

    QString idCaptureText;
    // Si on est deja sur la page Surpeche, l'ID est dans lineEdit_7c_7.
    if (ui->lineEdit_7c_7) {
        idCaptureText = ui->lineEdit_7c_7->text().trimmed();
    }
    // Fallback: champ de la page Captures principale (a cote du bouton Surpeche).
    if (idCaptureText.isEmpty() && ui->cap_lineEdit_7) {
        idCaptureText = ui->cap_lineEdit_7->text().trimmed();
    }

    if (idCaptureText.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("Surpeche"), QStringLiteral("Veuillez saisir l'ID Capture."));
        if (ui->lineEdit_7c_7) ui->lineEdit_7c_7->setFocus();
        else if (ui->cap_lineEdit_7) ui->cap_lineEdit_7->setFocus();
        return;
    }

    QString err;
    if (!loadSurpecheAnalysisForCapture(this, ui, idCaptureText, &err)) {
        QMessageBox::warning(this, QStringLiteral("Surpeche"), err.isEmpty() ? QStringLiteral("Rafraichissement echoue.") : err);
    }
}
...
void MainWindow::on_cap_btnShowTendance_clicked()
{
    ...
    // Analyse saisonnière: on ne filtre plus par bateau => masquer les champs ID bateau.
    ...
    // Remplacer l'affichage "jours" par "mois" sur l'axe X.
    {
        ...
        labels.reserve(7);
        for (int i = 6; i >= 0; --i) {
            const QDate m = endMonth.addMonths(-i);
            labels << monthShortFr(m.month());
        }

        for (int i = 0; i < 7; ++i) {
            const QString obj = QStringLiteral("d%1").arg(i + 1);
            if (auto* lab = tendancePage->findChild<QLabel*>(obj)) {
                lab->setText(labels.value(i));
            }
        }
    }
...
void MainWindow::on_cap_pushButton_4_clicked()
{
    ...
    const QString idBateauCol = matchColumnBySynonyms(cols, {
        QStringLiteral("id_bateau"), QStringLiteral("idbateau"), QStringLiteral("bateau_id")
    });
    const QString typeCol = matchColumnBySynonyms(cols, {
        QStringLiteral("type_poisson"), QStringLiteral("typepoisson"), QStringLiteral("poisson"), QStringLiteral("espece")
    ...
    if (idBateauCol.isEmpty() || typeCol.isEmpty() || quantiteCol.isEmpty() || poidsCol.isEmpty() || dateCol.isEmpty()) {
        QMessageBox::warning(this,
                             QStringLiteral("Saisonnalite"),
                             QStringLiteral("Colonnes CAPTURES manquantes (ID_BATEAU/TYPE/QUANTITE/POIDS/DATE)."));
        return;
    }

    // Définir les 7 mois affichés sur l'axe X (fin = mois de dateFin)
    const QDate endMonth(dateFin.year(), dateFin.month(), 1);
    QList<int> monthKeys; // year*12 + monthIndex(1..12)
    monthKeys.reserve(7);
    for (int i = 6; i >= 0; --i) {
        const QDate m = endMonth.addMonths(-i);
        monthKeys << (m.year() * 12 + m.month());
    }

    // Mettre à jour les labels d1..d7 avec les mois
    {
        auto monthShortFr = [](int month) -> QString {
            static const QStringList names = {
                QStringLiteral("Jan"), QStringLiteral("Fev"), QStringLiteral("Mar"), QStringLiteral("Avr"),
        ...
        for (int i = 0; i < 7; ++i) {
            const QDate m = endMonth.addMonths(-(6 - i));
            const QString obj = QStringLiteral("d%1").arg(i + 1);
            if (auto* lab = tendancePage->findChild<QLabel*>(obj)) {
                lab->setText(monthShortFr(m.month()));
            }
        }
    }

    // Totaux sur toute la période sélectionnée (en mois, pas en jours)
    double totalPoids = 0.0;
    int totalQuantite = 0;
    {
        QStringList where;
        ...
        QSqlQuery q(db);
        q.prepare(sql);
        q.addBindValue(dateDebut.toString(QStringLiteral("yyyy-MM-dd")));
        q.addBindValue(dateFin.toString(QStringLiteral("yyyy-MM-dd")));
        if (!species.isEmpty()) {
            q.addBindValue(species.toUpper());
        }

        if (q.exec() && q.next()) {
            totalPoids = q.value(0).toDouble();
            totalQuantite = q.value(1).toInt();
        }
    }

    // Agrégation par mois (pour tooltips sur les points)
    ...
    QHash<int, int> qtyByMonth;
    {
        ...
        if (q.exec()) {
            while (q.next()) {
                const QDate m = q.value(0).toDate();
                if (!m.isValid()) continue;
                const int key = (m.year() * 12 + m.month());
                poidsByMonth.insert(key, q.value(1).toDouble());
                qtyByMonth.insert(key, q.value(2).toInt());
            }
        }
    }
    ...
    // Rendre le graphique "fonctionnel":
    // - Axe Y adapté au max mensuel
    // - Points (p1..p7) dimensionnés selon le poids du mois
    double maxMonthly = 0.0;
    for (int i = 0; i < 7; ++i) {
        maxMonthly = qMax(maxMonthly, poidsByMonth.value(monthKeys.value(i), 0.0));
    }
    if (maxMonthly <= 0.0) {
        maxMonthly = 1.0;
    }

    {
        ...
        const int ticks = yNames.size();
        const double step = (ticks > 1) ? (maxMonthly / (ticks - 1)) : maxMonthly;
        for (int i = 0; i < ticks; ++i) {
            const double v = maxMonthly - (i * step);
            if (auto* lab = tendancePage->findChild<QLabel*>(yNames.at(i))) {
                lab->setText(QStringLiteral("%1 KG").arg(QString::number(qMax(0.0, v), 'f', 0)));
            }
        }
    }
    ...
    auto extractColor = [](const QString& styleSheet) -> QString {
        ...
        const QRegularExpressionMatch m = rx.match(styleSheet);
        if (m.hasMatch()) {
            return m.captured(1).trimmed();
        }
        return QStringLiteral("#4fc3f7");
    };

    for (int i = 0; i < 7; ++i) {
        const int key = monthKeys.value(i);
        const double p = poidsByMonth.value(key, 0.0);
        const int qv = qtyByMonth.value(key, 0);
        const QString obj = QStringLiteral("p%1").arg(i + 1);
        if (auto* pt = tendancePage->findChild<QLabel*>(obj)) {
            const double ratio = qBound(0.0, p / maxMonthly, 1.0);
            const int barLines = qBound(1, 1 + static_cast<int>(ratio * 8.0 + 0.5), 9);
            QStringList lines;
            lines.reserve(barLines);
            for (int j = 0; j < barLines; ++j) {
                lines << QStringLiteral("█");
            }

            pt->setText(lines.join(QStringLiteral("\n")));
            pt->setAlignment(Qt::AlignHCenter | Qt::AlignBottom);
            pt->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Expanding);
            pt->setToolTip(QStringLiteral("Poids: %1 kg\nQuantite: %2").arg(QString::number(p, 'f', 2), QString::number(qv)));

            const QString color = extractColor(pt->styleSheet());
            pt->setStyleSheet(QStringLiteral("color:%1; font-size:14px;").arg(color));
        }
    }
}
